...
export default function Page() {
    ...
    useEffect(() => {
        async function fetchTransactions() {
            ...
            const canteenTx: Transaction[] = (canteenData.txCanteen || [])
                .filter((tx: any) => tx.txType === 'pengeluaran' && tx.date.startsWith(monthStr))
                .map((tx: any) => ({
                    id: tx.id,
                    txType: tx.txType,
                    txTitle: tx.txTitle || '-',
            ...
            const foTx: Transaction[] = (foData.txFo || [])
                .filter((tx: any) => tx.type === 'pengeluaran' && tx.date.startsWith(monthStr))
                .map((tx: any) => ({
                    id: tx.id + 1000,
                    txType: tx.type,
                    txTitle: tx.title || '-',
    ...
    const handleDeleteTx = () => {
        if(selectedTx){
            const updatedTx = mockTx.filter((tx) => tx.id !== selectedTx.id);
            setMockTx(updatedTx);
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    ...
                                    <BreadcrumbSeparator />
                                    <BreadcrumbLink href="/acc" className="text-gray-600 hover:text-gray-900">Accounting</BreadcrumbLink>
                                    <BreadcrumbSeparator />
                                    <BreadcrumbPage>Pengeluaran Bulan Ini</BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
            ...
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                <DialogContent>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <Label>Metode Pembayaran</Label>
                            <Input value={txPaymentMethod} onChange={(e) => setTxPaymentMethod(e.target.value)} />
                        </div>
                    </div>
                    <DialogFooter>
