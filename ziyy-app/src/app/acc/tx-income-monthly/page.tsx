    ...
    useEffect(() => {
        async function fetchTransactions() {
            ...
            const canteenTx: Transaction[] = (canteenData.txCanteen || [])
                .filter((tx: any) => tx.txType === 'pemasukan' && tx.date.startsWith(monthStr))
                .map((tx: any) => ({
                    id: tx.id,
                    txType: tx.txType,
                    txTitle: tx.txTitle || '-',
            ...
            const foTx: Transaction[] = (foData.txFo || [])
                .filter((tx: any) => tx.type === 'pemasukan' && tx.date.startsWith(monthStr))
                .map((tx: any) => ({
                    id: tx.id + 1000,
                    txType: tx.type,
                    txTitle: tx.title || '-',
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    ...
                                    <BreadcrumbSeparator />
                                    <BreadcrumbPage>Pemasukan Bulan Ini</BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
