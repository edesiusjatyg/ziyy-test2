...
export default function Page() {
    ...
    const loadData = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const incomeTx = transactions.filter((tx: Transaction) => {
    ...
    const calculatePaymentAmount = () => {
        if (txItemId && txItemAmount) {
            const selectedItem = canteenItems.find(item => item.id.toString() === txItemId);
            if (selectedItem) {
                const calculatedAmount = selectedItem.price * parseInt(txItemAmount);
                setTxPaymentAmount(calculatedAmount.toString());
            }
        }
    };
    ...
    }, [txItemId, txItemAmount, canteenItems]);
        ...
        if (selectedTx) {
            try {
                const updateData = {
                    title: txTitle,
                    note: txNote,
                    itemId: parseInt(txItemId),
                    itemAmount: parseInt(txItemAmount),
    ...
    const getItemBadge = (itemName: string) => {
        const name = itemName.toLowerCase();
        
        if (name.includes("cleo")) {
            return <Badge className="bg-orange-100 text-orange-800 hover:bg-orange-100">{itemName}</Badge>;
        } else if (name.includes("bcaa") || name.includes("creatine") || name.includes("protein")) {
            return <Badge className="bg-sky-100 text-sky-800 hover:bg-sky-100">{itemName}</Badge>;
        } else if (name.includes("hilo") || name.includes("tropicana") || name.includes("milk")) {
            return <Badge className="bg-green-100 text-green-800 hover:bg-green-100">{itemName}</Badge>;
        } else if (name.includes("americano") || name.includes("latte") || name.includes("coffee")) {
            return <Badge className="bg-red-100 text-red-800 hover:bg-red-100">{itemName}</Badge>;
        } else {
            return <Badge className="bg-gray-100 text-gray-800 hover:bg-gray-100">{itemName}</Badge>;
        }
    }
              ...
              <Breadcrumb>
                <BreadcrumbList>
                  <BreadcrumbItem>
                    ...
                    <BreadcrumbPage className="text-white text-sm md:text-base">Pemasukan</BreadcrumbPage>
                  </BreadcrumbItem>
                </BreadcrumbList>
              </Breadcrumb>
            </div>
                  ...
                  >
                    <CardHeader>
                      <CardTitle className="text-gray-900 text-base flex items-center gap-2">
                        {getItemBadge(tx.item?.name || "Unknown")}
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                    ...
                    {selectedTx && (
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-semibold">{getItemBadge(selectedTx.item?.name || "Unknown")}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-sm">{selectedTx.itemAmount}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-sm">{getPaymentMethodBadge(selectedTx.paymentMethod)}</p>
                            </div>
                            </div>
                        </div>
                    )}
                ...
                <div className="space-y-4">
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="txItemId" className="text-right">
                        Item
                      </Label>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <p className="text-sm font-semibold">
                          {formatCurrency(parseInt(txPaymentAmount))}
                      </p>
                    </div>
                  </div>
                </div>
